...
Checks an individual against AML datasets (e.g. PEP and sanctions) using the new provider’s search service.  
...
| `countries` | string  | No       | Comma-separated list of two-letter ISO-3166 country codes affecting both address and nationality. Example: `"NL,GB"`. |
| `datasets`  | string  | Yes      | Comma-separated list specifying which datasets will be searched. See **Datasets** below. Example: `"PEP,SAN"`. |
...
| `data.id`    | string | Provider-specific identifier of the person profile. |
| `data.attributes` | object | Detailed attributes describing the person, risk exposure, and linked entities. |
| `data.links` | object or null | Optional hypermedia links – often `null` for this integration. |
</Accordion>
...
| `isDeleted`                  | boolean       | Indicates whether the profile has been logically deleted in the provider system. |
| `monitoringId`               | string        | Identifier used by the provider for ongoing monitoring of this profile. |
